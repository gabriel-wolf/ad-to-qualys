...
Active Directory OUs
        |
        v
Eligible computers identified
        |
        v
Workstation or server AD group updated
        |
        v
Hostnames resolved against Qualys Asset Management
        |
        v
Qualys asset IDs collected
        |
        v
Patch-management tag applied in bulk
        |
        v
Assets enter the appropriate Qualys patch scope
```
...
├── .gitignore
└── imgs
    ├── get-qualys-asset-success.png
    └── automation-success.png
```
...
![Successful Qualys asset lookup](imgs/get-qualys-asset-success.png)
...
![Successful AD-to-Qualys automation run](imgs/automation-success.png)

> Update the image paths above if the screenshots in the `imgs` directory use different filenames.
...
```powershell
$QualysUsername = "QUALYS_API_USERNAME"
$QualysPlatform = "qualysapi.example.com"
$SecretPath     = "C:\ProgramData\QualysAutomation\qualys_password.enc"

$DnsSuffix        = "example.com"
$OUMenuSearchBase = "DC=example,DC=com"

$WorkstationADGroupDN = "CN=Workstation-Patch-Management,OU=Security Groups,DC=example,DC=com"
$ServerADGroupDN      = "CN=Server-Patch-Management,OU=Security Groups,DC=example,DC=com"
...
Populate `workstation_ou.txt` and `server_ou.txt` with the OU names that should be evaluated.
...
```text
-NoProfile -ExecutionPolicy Bypass -File "C:\Scripts\Qualys\Automation.ps1" -TargetMode Workstation
```

A separate task can run server mode:

```text
-NoProfile -ExecutionPolicy Bypass -File "C:\Scripts\Qualys\Automation.ps1" -TargetMode Server
```
...
### Additive Onboarding Behavior

The current implementation adds eligible computers to the configured Active Directory group and adds the appropriate Qualys tag.

It does not automatically:

* Remove computers from the Active Directory group when they leave a configured OU
* Remove Qualys tags from assets that are no longer eligible
* Delete stale or duplicate Qualys asset records
* Approve or deploy specific patches
* Replace Qualys patch-job scheduling or deployment controls

This project automates asset onboarding and scope assignment. Patch approval, scheduling, maintenance windows, reboot behavior, deployment policies, and exception management remain controlled through the organization’s Qualys Patch Management configuration.
...
* Moving environment-specific values into a separate configuration file
* Adding `SupportsShouldProcess` and a dry-run mode
* Implementing removal or reconciliation logic
* Adding retry handling for transient API failures
* Escaping XML values through an XML writer rather than string interpolation
* Adding pagination handling for larger Qualys responses
* Adding automated tests for hostname normalization and OS filtering
* Producing structured JSON or CSV execution reports
* Integrating with a centralized secret-management platform
* Sending execution summaries to a SIEM, email, or collaboration platform
